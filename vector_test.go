...
func TestVector_Add(t *testing.T) {
	...
	testVector(t, Vec(1, 2).AddXY(3, -2), 4, 0)
	testVector(t, Vec(0.4, -0.25).Add(Vec(100.1, -0.1)), 100.5, -0.35)
	testVector(t, Vec(0.4, -0.25).AddXY(100.1, -0.1), 100.5, -0.35)
}

func TestVector_Sub(t *testing.T) {
	testVector(t, Vec(1, 2).Subtract(Vec(3, -3)), -2, 5)
	testVector(t, Vec(1, 2).SubtractXY(3, -3), -2, 5)
	testVector(t, Vec(0.4, -0.25).Subtract(Vec(100.1, -0.1)), -99.7, -0.15)
	testVector(t, Vec(0.4, -0.25).SubtractXY(100.1, -0.1), -99.7, -0.15)
}

func TestVector_Multiply(t *testing.T) {
	testVector(t, Vec(1, 2).Multiply(3), 3, 6)
	testVector(t, Vec(1, 2).MultiplyXY(3, 2), 3, 4)
	testVector(t, Vec(0.4, -0.25).Multiply(-1.5), -0.6, 0.375)
	testVector(t, Vec(0.4, -0.25).MultiplyXY(-1.5, 2), -0.6, -0.5)
}

func TestVector_Divide(t *testing.T) {
	testVector(t, Vec(5, 10).Divide(2), 3, 5)
	testVector(t, Vec(5, 10).DivideXY(3, 2), 2, 5)
	testVector(t, Vec(0.4, -0.25).Divide(-2), -0.2, 0.125)
	testVector(t, Vec(0.4, -0.25).DivideXY(-4, 0.5), -0.1, -0.5)
}
...
func TestVector_Abs(t *testing.T) {
	testVector(t, Vec(1, 2).Abs(), 1, 2)
	testVector(t, Vec(-1, 3).Abs(), 1, 3)
	testVector(t, Vec(-0.4, -0.25).Abs(), 0.4, 0.25)
}
...
func TestVector_Normal(t *testing.T) {
	testVector(t, Vec(5, 10).Normal(), -10, 5)
	testVector(t, Vec(0.4, -0.25).Normal(), 0.25, 0.4)
}
...
func TestVector_Zero(t *testing.T) {
	assert.False(t, Vec(1, 2).IsZero())
	assert.True(t, Vec(0, 0).IsZero())
	assert.True(t, ZeroVector[int]().IsZero())

	assert.False(t, Vec(0.4, -0.25).IsZero())
