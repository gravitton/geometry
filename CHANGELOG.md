...
- `LerpAngle(a, b, t)` – interpolation from one angle towards another along the shorter arc, so a turn from 350° to 10° passes through 0°; extrapolates outside `[0, 1]` like `Lerp`, and turns by increasing angle where the angles are half a turn apart
- `Size.Transpose()` – the width and height swapped, the size of the same extent turned a quarter turn
...
- `Circle.DistanceTo(point)`, `Rectangle.DistanceTo(point)` and `Polygon.DistanceTo(point)` – the distance to the nearest point of the shape, zero exactly where `Contains` holds, the tolerance band included, so every shape measures distance the way `Line` does; an empty polygon is infinitely far, having no point to measure to
...
- `Line.IntersectionCircle(circle)`, `IntersectionRectangle(rectangle)` and `IntersectionPolygon(polygon)`, with `Circle.IntersectionLine`, `Rectangle.IntersectionLine` and `Polygon.IntersectionLine` as mirrors – the points where a segment crosses the boundary, from `Start` to `End`: two through a circle or rectangle, one tangent or ending inside, none apart or entirely inside, and as many as a concave polygon is crossed; a vertex hit by two edges is counted once, and an endpoint within `Epsilon[T]()` of a boundary is a crossing judged on its distance like the `Intersects` test, so a shallow touch is not lost to the fraction along the chord; a chord shorter than `Epsilon[T]()` is a tangent and gives its midpoint, while a longer one keeps both ends even where it grazes the boundary within the tolerance, so the tolerance never turns two crossings into one
- `Size.Abs()` – the absolute size, which every `Rectangle` constructor stores
- `Rectangle.Canonical()`, `Circle.Canonical()` and `RegularPolygon.Canonical()` – the shape in the form `Rect`, `Circ` and `RegPol` build, with its size or radius taken absolute and, on the regular polygon, the angle normalized as `Rotate` stores it, so a negative extent written as a struct literal or decoded from JSON can be repaired before `Contains` and the `Intersects` methods read it; a well-formed shape is returned as it is. Named after `image.Rectangle.Canon`, in full like `Subtract`, rather than `Abs`, which on `Point`, `Vector` and `Size` is component-wise and here would leave the center alone
- `Circle.Intersection(circle)` – the points where two circles cross: two for overlapping, one for tangent within `Epsilon[T]()`, none for apart, nested, concentric or coincident within `Epsilon[T]()`; tangency is judged by the comparison that admits the circles, so a tangent exactly `Epsilon[T]()` outside its reach gives one point where `Intersects` holds, not the same point twice
...
- `Rectangle.Unscale` and `UnscaleXY`, `Circle.Unscale`, `Polygon.Unscale` and `UnscaleXY`, `RegularPolygon.Unscale` and `UnscaleXY` – the inverse of `Scale`, so every shape has the pair `Size`, `Padding` and `Matrix` already had. It is not `Scale(1/factor)`: each extent is divided once rather than multiplied by a reciprocal, and a zero factor panics through `Divide` instead of casting an infinity
- `Size.Round`, `Floor`, `Ceil` and `Lerp` – the arithmetic `Point` and `Vector` already had, so a size can be snapped to the pixel grid or interpolated
- `Line.Angle` and `Direction` – the angle of the segment from `Start` to `End` in radians and the nearest of the eight directions, as `Vector.Angle` and `Vector.Direction` judge the segment's vector; a zero-length segment has no direction and gives `0` and `DirectionNone`
- `Matrix.Shear`, `PreShear`, `Reflect` and `PreReflect` – compose with `ShearMatrix` and `ReflectionMatrix` on either side, so all five affine constructors have the composing methods `Translate`, `Rotate` and `Scale` already had; reflecting across `AxisNone` leaves the matrix unchanged
- `Matrix.IsIdentity` – reports whether the matrix leaves every point where it is, comparing like `Equal`
- `RegularPolygon.Area` and `Perimeter` – measured from the angles without building the vertices, so for an integer `T` they are the area and perimeter of the exact polygon rather than of the rounded one: the area in closed form, `n/2 · w · h · sin(2π/n)` for the polygon inscribed in the ellipse `Size` describes; the perimeter as `2 · n · r · sin(π/n)` on a square `Size` and as the sum of the chords on an ellipse, where it has no closed form
- `Padding.Subtract` – the counterpart of `Add`
- `Vector.AtMost(length)` and `AtLeast(length)` – cap and floor the magnitude along the vector's own direction, judged like `LessOrEqual` and `Less`, so a velocity is held to a top speed in one call; a non-positive `AtMost` gives the zero vector and a non-positive `AtLeast` changes nothing
- `Line.Normal()` – the perpendicular of the segment with its length, `Vector.Normal` of its vector, pointing inward on a `Rectangle` edge or any polygon wound the same way
- `Circle.AlignTo(direction, point)` – moves the circle so its `Anchor` lands on the point, the inverse of `Anchor` as on `Rectangle`
...
- `Rectangle.Lerp`, `Circle.Lerp` and `Padding.Lerp` – interpolation towards another shape, extrapolating outside `[0, 1]` like `Point.Lerp`, so a shape can be tweened. The rectangle moves its center and size together and keeps the size absolute, and the circle moves its center and radius and keeps the radius absolute. `Polygon` and `Matrix` are left out: two polygons need not share a vertex count, and a component-wise matrix blend is not the interpolation of a transform
- `RegularPolygon.Lerp` – moves the center and the size together and turns the angle along the shorter arc with `LerpAngle`, normalized like `Rotate`; `N` does not interpolate, so polygons with a different vertex count have no shape between them and `Lerp` panics for them, like `RegularPolygonOrientationAngle` for an unknown orientation
...
- `Matrix` documents which composing methods take their constructor's arguments and that `Scale` and `Unscale` take `float64` factors where `ScaleMatrix` takes `T`
- `Circle.Scale` and `RegularPolygon.Scale` take a negative factor absolute, like `Rectangle.Scale`: a shape mirrored about its own center is the same shape, so `Circ(Pt(0, 0), 5).Scale(-1)` keeps its radius. `Polygon.Scale` and `Line.Scale` keep the flip, where it is a genuine point reflection about the centroid or the midpoint and no stored extent goes negative (**breaking**)
- `Circle` never stores a negative radius, like `Rectangle` its size: `Circ`, `Resize` and `Lerp` take the radius absolute, so a negative one can only be written as a struct literal or decoded from JSON, and `Circle.Canonical` repairs that. The "empty circle" a negative radius described, which contained nothing, intersected nothing, anchored at its center and bounded a zero size, is gone with the seven special cases that carried it: `Anchor`, `Bounds`, `DistanceTo`, `Intersects`, `Intersection`, `Line.IntersectionCircle` and `Polygon.IntersectsCircle` treat every radius alike (**breaking** for a caller relying on a negative radius as an empty circle). Every method that could produce a negative extent, `Scale`, `ScaleXY`, `Unscale`, `UnscaleXY`, `Resize`, `Lerp` and `Canonical` on `Rectangle`, `Circle` and `RegularPolygon`, takes its result absolute the way `Rectangle.Scale` already did, rather than the factor
...
- `Size.Fit` and `Fill` document that a negative extent is outside the contract of `Size` and fits or fills to a negative one, which bounds and covers nothing; the docs covered the zero extent and were silent on the negative one
- `Direction.Vector` documents that a negative length points the other way, as `Vector.Resize` gives it; `Vector.Resize` said so and the method callers reach for did not
...
- `RegularPolygon.Bounds` returns the zero rectangle for `N < 1`, the same answer `Polygon.Bounds` gives for no vertices, instead of a zero-size rectangle at the center (**breaking**), and is placed from the four vertices nearest to the axis directions without building the vertex slice, reading each as `Vertices` places it so the box is bit for bit the one `Polygon().Bounds()` gave
