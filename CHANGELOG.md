...
- `Rectangle.MinMax()` – both corners at once, for the methods that need the pair
...
- `Line.Intersection(line)` – the point where two segments cross and whether they do; parallel segments have no single point and report false even where they overlap; whether they cross is decided on the fractions along each segment, from cross products that are exact for an integer `T`, so it answers exactly where `Intersects` holds for non-parallel segments
...
- `Line.DistanceToLine(line)` – the distance between the nearest points of two segments, zero where they cross; `Intersects` is this within `Epsilon[T]()`
...
- `Vector.Dot` and `Cross` round their two products separately, so a fused multiply-add cannot turn the dot product of perpendicular vectors or the cross product of parallel ones into a rounding error: `Vec(13.5, 1.9).Cross(Vec(13.5, 1.9))` is exactly `0` instead of `-2.2e-16`; `Polygon.Area` and `Center` rely on it for degenerate edges
