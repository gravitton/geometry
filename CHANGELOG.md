...
- `Circle.DistanceTo(point)`, `Rectangle.DistanceTo(point)` and `Polygon.DistanceTo(point)` – the distance to the nearest point of the shape, zero exactly where `Contains` holds, the tolerance band included, so every shape measures distance the way `Line` does; an empty polygon and a circle with a negative radius are infinitely far, having no point to measure to
...
- `Circle.Intersection(circle)` – the points where two circles cross: two for overlapping, one for tangent within `Epsilon[T]()`, none for apart, nested, concentric, coincident within `Epsilon[T]()` or a negative radius; tangency is judged by the comparison that admits the circles, so a tangent exactly `Epsilon[T]()` outside its reach gives one point where `Intersects` holds, not the same point twice
...
- `Circle.Scale` and `RegularPolygon.Scale` take a negative factor absolute, like `Rectangle.Scale`: a shape mirrored about its own center is the same shape, and the sign of a radius says whether a circle is empty rather than which way it faces, so `Circ(Pt(0, 0), 5).Scale(-1)` keeps its radius instead of becoming the empty circle that contains and intersects nothing. A deliberately negative radius stays negative, since the factor is what is made absolute, not the result. `Polygon.Scale` and `Line.Scale` keep the flip, where it is a genuine point reflection about the centroid or the midpoint and no stored extent goes negative (**breaking**)
